...
- Tool arguments are now validated at runtime against the declared JSON-Schema (required fields, types, additionalProperties, min/max/enum) before dispatch; invalid arguments short-circuit to a structured error with a recovery hint instead of executing, turning the previously-declared-but-unenforced schemas into a real input-contract check (shift-left / contract testing).
- The project now has a static-analysis gate: `scripts/verify.ps1` runs `ruff check` + `mypy` (lenient) alongside the existing compileall + pytest, configured in `pyproject.toml`. Ruff caught a real latent bug on first run (`main_window.py` used `re` without importing it in a never-called path) plus unused imports/variables; mypy runs in a lenient mode (UI and noisy annotation-heavy codes excluded) so the gate stays green without a full annotation pass.
